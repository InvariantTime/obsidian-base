...
> 1. При container escape атакующий получает **root на хост-машине**
> 2. Файлы, созданные контейнером, принадлежат root на хосте
> 3. Нарушает принцип минимальных привилегий
> 4. Многие уязвимости (CVE) эксплуатируются именно через root
...
**Docker по умолчанию выдаёт** ограниченный набор и **дропает** опасные.
...
**Seccomp** (Secure Computing Mode) — whitelist/blacklist системных вызовов.
