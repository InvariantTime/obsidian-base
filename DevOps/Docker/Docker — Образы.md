...
**Docker Image** — read-only шаблон, состоящий из набора **слоёв (layers)**.  
Каждый слой — это diff файловой системы относительно предыдущего.
...
│ COPY . /app  (layer 4)          │
├─────────────────────────────────┤
│ RUN npm ci   (layer 3)          │  Read-only layers
├─────────────────────────────────┤
│ COPY package.json (layer 2)     │  (из Dockerfile)
├─────────────────────────────────┤
...
> 5. Используй `--squash` для объединения всех слоёв (экспериментальная функция)
