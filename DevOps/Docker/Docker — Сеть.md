...
| ----------- | ----------------------------------------------------------------------------- | --------------------------------------------------- |
| **bridge**  | Виртуальная сеть на хосте (по умолчани                                         | Изолированные контейнеры на одном хосте             |
| **host**    | Использует сеть хоста напря                                                    | Максимальная производительность, нет изоляции       |
| **none**    | Нет                                                                            | Полностью изолированный контейнер                   |
| **overla Объединение контейнеров на разных физических серверах в одну логическую сеть. сеть. | Docker Swarm, мультихостовые сети                   |
| **macvlan** | Контейнер получает MAC-адрес как физическое устр                               | Legacy-приложения, требующие прямого доступа к сети |
| **ipvlan**  | Аналог macvlan, но без MAC-                                                    | Аналогично macvlan                                  |

---

## bridge — по умолчанию

![[excalidraw-docker-network-base]]
...
```bash
# Псевдонимы для DNS
docker run --network mynet --network-alias database postgres
# теперь доступно и как "postgres", и как "database"
```
...
# Linux:
host.docker.internal    # может не работать на Linux без доп. настройки
172.17.0.1             # обычно gateway bridge сети
