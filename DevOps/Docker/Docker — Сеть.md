...
|---------|---------|---------|
| **bridge** | Виртуальная сеть на хосте (по умолчанию) | Изолированные контейнеры на одном хосте |
| **host** | Использует сеть хоста напрямую | Максимальная производительность, нет изоляции |
| **none** | Нет сети | Полностью изолированный контейнер |
| **overlay** | Сеть между несколькими Docker-хостами | Docker Swarm, мультихостовые сети |
| **macvlan** | Контейнер получает MAC-адрес как физическое устройство | Legacy-приложения, требующие прямого доступа к сети |
| **ipvlan** | Аналог macvlan, но без MAC-адресов | Аналогично macvlan |
